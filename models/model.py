...
class Palette(BaseModel):
    def __init__(self, networks, losses, sample_num, task, optimizers, ema_scheduler=None, **kwargs):
        ...
        # default: Definition of the backbone - Unet
    ...
    # default: the training process of palette model
    def train_step(self):
        self.netG.train()
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for phase_data in tqdm.tqdm(self.phase_loader):
                ...
                #     self.writer.add_images(key, value)
                # # modify: to reduce the space used, so we remove it
                self.writer.save_images(self.save_current_results())
                # if(image_flag):
                #     self.writer.save_images(self.save_current_results())
                #     image_flag = False
        ...
        time_end = time.time()
        self.logger.info('The test FPS is: {}'.format(len(self.phase_loader.dataset)/(time_end-time_start)))
    ...
    # default: load the pretrained model. If it doesn't exist, then do nothing
